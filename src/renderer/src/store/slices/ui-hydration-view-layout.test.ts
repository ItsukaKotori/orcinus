...
describe('sidebar reveal actions', () => {
  it('queue the reveal request for the worktree list', () => {
...
describe('createUISlice hydratePersistedUI', () => {
  ...
  it('restores a persisted plugin-center top-level view on hydration', () => {
    const store = createUIStore()

    store.getState().hydratePersistedUI(makePersistedUI({ activeView: 'plugin-center' }), 'startup')

    expect(store.getState().activeView).toBe('plugin-center')
  })

  it('restores a persisted plugin-hosted top-level view on hydration', () => {
    const store = createUIStore()

    store
      .getState()
      .hydratePersistedUI(
        makePersistedUI({ activeView: 'plugin:database-manager.main' }),
        'startup'
      )

    expect(store.getState().activeView).toBe('plugin:database-manager.main')
  })
  ...
  it('keeps a persisted full-page view when the settings fetch failed', () => {
    ...
    store.getState().hydratePersistedUI(makePersistedUI({ activeView: 'space' }), 'startup')
