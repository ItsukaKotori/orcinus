...
describe('automation host recovery', () => {
  ...
  it('deep-links update-server to Automations after the remote host panes were removed', () => {
    const target = deps()
    runAutomationHostRecovery('update-server', RUNTIME_SSH, target)
    expect(target.openSettings).toHaveBeenCalledWith({ pane: 'automations', repoId: null })

    target.openSettings.mockClear()
