...
describe('AppearancePane', () => {
  ...
  it('records Resource Manager status bar toggles as resource-manager interactions', async () => {
    mocks.state.availableStatusBarToggles = [
      {
        id: 'resource-usage',
        title: 'Resource Manager',
        description: 'Show CPU, memory, terminal sessions, and workspace disk usage.',
        toggleDescription: 'Show the Resource Manager.',
        keywords: ['status bar', 'resource']
      }
    ]
    mocks.state.settingsSearchQuery = 'resource'
    const container = await renderAppearancePane(getDefaultSettings('/tmp'))
    const resourceSwitch = container.querySelector<HTMLButtonElement>(
      'button[role="switch"][aria-label="Resource Manager"]'
    )

    expect(resourceSwitch).not.toBeNull()
    await act(async () => {
      resourceSwitch?.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    })

    expect(mocks.state.recordFeatureInteraction).toHaveBeenCalledWith('resource-manager')
    expect(mocks.state.toggleStatusBarItem).toHaveBeenCalledWith('resource-usage')
  })
