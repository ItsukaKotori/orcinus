...
function findRunTargetItem(label: string): HTMLElement | undefined {
  return [...document.body.querySelectorAll<HTMLElement>('[role="option"]')].find((item) =>
...
describe('NewWorkspaceComposerCard folder task source mode', () => {
  ...
  it('shows the run target picker for one ready setup without the removed add-host row', () => {
    current = renderCard({
    ...
    expect(current.container.textContent).toContain('Run on')
    openRunTargetPicker(current.container)
    expect(findRunTargetItem('Local')).toBeTruthy()
    expect(findRunTargetItem('Add host')).toBeFalsy()
  })
  ...
  it('keeps the removed add-host entries out of the run target picker', () => {
    current = renderCard({
      projectHostSetupOptions: [localReadyHostOption, devboxNeedsSetupHostOption],
      selectedProjectHostSetupId: 'setup-local'
    })
    ...
    expect(findRunTargetItem('Add host')).toBeFalsy()
    expect(findRunTargetItem('Add SSH host')).toBeFalsy()
    expect(findRunTargetItem('Add Remote Orca Server')).toBeFalsy()
    expect(document.body.querySelector('[data-testid="add-remote-host-dialog"]')).toBeNull()
  })
