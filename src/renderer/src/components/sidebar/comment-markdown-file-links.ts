...
function isLinkifiableFile(link: ParsedTerminalFileLink, requireSeparator: boolean): boolean {
  ...
  return (
    ...
    routeMarkdownHref(link.displayText).kind === 'file'
  )
}
...
function createFileLinkNode(value: string, child: MarkdownNode): MarkdownNode {
  return {
    type: 'link',
    url: createMarkdownFileHref(value),
    children: [child]
  }
}
...
function transformFileLinks(node: MarkdownNode): void {
  if (node.type === 'link') {
    if (node.url && routeMarkdownHref(node.url).kind === 'file') {
      node.url = createMarkdownFileHref(node.url)
    }
    return
  }
...
export function remarkMarkdownFileLinks(): (tree: MarkdownNode) => void {
