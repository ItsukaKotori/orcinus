...
describe('buildHostHeaderMenuModel', () => {
  ...
  it('offers Check connection without Remove for a runtime host', () => {
  ...
  it('offers Remove only for ssh hosts', () => {
    expect(buildHostHeaderMenuModel({ kind: 'ssh', health: 'available' }).actions).toContain(
      'remove'
    )
    expect(buildHostHeaderMenuModel({ kind: 'runtime', health: 'available' }).actions).not.toContain(
      'remove'
    )
