...
export type PaneTitleAgentPane = {
  id: number
  leafId: string
}

export type PaneTitleAgentInput = {
  leafId: string | null
  panes: readonly PaneTitleAgentPane[]
  runtimePaneTitlesByPaneId: Readonly<Record<number, string>>
  tabLabel?: string | null
  terminalTitle?: string | null
}

export function resolvePaneTitleAgent({
  leafId,
  panes,
  runtimePaneTitlesByPaneId,
  tabLabel,
  terminalTitle
}: PaneTitleAgentInput): TuiAgent | null {
  if (!leafId) {
    return null
  }
