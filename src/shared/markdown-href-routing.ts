...
export const MARKDOWN_FILE_HREF_PREFIX = '#orca-file-link='
const MAX_MARKDOWN_FILE_HREF_DECODES = 4

export function createMarkdownFileHref(pathText: string): string {
  return `${MARKDOWN_FILE_HREF_PREFIX}${encodeURIComponent(pathText)}`
}

function decodeMarkdownFileHref(href: string): string | null {
  if (!href.startsWith(MARKDOWN_FILE_HREF_PREFIX)) {
    return null
  }
  try {
    const decoded = decodeURIComponent(href.slice(MARKDOWN_FILE_HREF_PREFIX.length))
    return decoded && !decoded.startsWith(MARKDOWN_FILE_HREF_PREFIX) ? decoded : null
  } catch {
    return null
  }
}
...
export function routeMarkdownHref(href: string | null | undefined): MarkdownHrefRoute {
  let trimmed = href?.trim()
  if (!trimmed) {
    return { kind: 'none' }
  }
  for (let depth = 0; depth < MAX_MARKDOWN_FILE_HREF_DECODES; depth += 1) {
    const encodedFileHref = decodeMarkdownFileHref(trimmed)
    if (!encodedFileHref) {
      break
    }
    trimmed = encodedFileHref.trim()
  }
  if (!trimmed || trimmed.startsWith(MARKDOWN_FILE_HREF_PREFIX)) {
    return { kind: 'none' }
  }
