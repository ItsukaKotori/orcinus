...
// Why: static views live in StaticTopLevelView's exhaustive lookup and plugin-hosted
// views are open-ended (`plugin:<pluginKey>`); reusing the shared split keeps ui.set in
// lockstep with hydration instead of maintaining a second enum that can drift.
export const TopLevelViewSchema = z.custom<TopLevelView>(isTopLevelView, {
  message: 'Unknown top-level view'
})
